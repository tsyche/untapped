...
- Generic sources: conf field 2 may be an `https://` URL (version discovery) with a full-URL `asset_pattern` in field 3 and optional `version_rule` regex as the last field (may contain `|` — it's the last field, so `read` keeps the remainder). Checksums skipped (GitHub convention only). `add` handles them: version pages are scraped for archive links — candidates get numbered selection (`--yes` takes the best version/OS/arch match, `--asset` forces a full URL), pages with no usable links fall back to a manual prompt or editable template; a direct download URL is one-shot — version from the URL, source from the parent dir (version segment stripped), name/filters from the filename, prompt or editable template only when discovery fails.
