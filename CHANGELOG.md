...
### Added
- `GH_TOKEN` accepted as a fallback when `GITHUB_TOKEN` is unset (matches `gh`'s env); still sent to GitHub hosts only
...
- Commands: bare `untapped` (install missing + upgrade behind), `upgrade` (alias), `list`, `doctor`, `outdated`, `lint`, `why <name>`, `add <url|o/r>`, `remove`, `help`; `bin/ut` symlink ships as a second name
